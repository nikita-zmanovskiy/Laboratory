...
export const SendButton = ({
    ...
    const activeBgClass = isTextMode
        ? "bg-[#9a2e49] hover:bg-[#b43f5c]"
        : "bg-[#2158b0] hover:bg-[#4575c2]"

    return (
        <button
            type="submit"
            disabled={isDisabled || isLoading}
            className={`flex h-10 w-10 cursor-pointer items-center justify-center rounded-full text-[var(--color-text-primary)] shadow-md transition-all duration-200 hover:scale-105 focus:outline-none active:scale-95 disabled:scale-100 disabled:cursor-not-allowed disabled:bg-[var(--color-bg-primary)] disabled:text-[var(--color-text-muted)] ${activeBgClass}`}
        >
            {isLoading ? (
