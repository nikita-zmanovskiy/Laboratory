...
export class ClassroomController {
    ...
    /** Учитель с валидной lab_teacher cookie получает отдельную student-сессию для чата (не затирает учителя) */
    ...
    deactivate = async (req: Request, res: Response, next: NextFunction) => {
        try {
            ...
            const code = req.params.code as string
            console.log("[Deactivate] Starting for code:", code)
            ...
            console.log("[Deactivate] Deactivated in DB, broadcasting...")
            ...
            const wsService = getWebSocketService()
            console.log("[Deactivate] wsService:", !!wsService)
            if (wsService) {
                wsService.broadcastClassroomClosed(code as string, 'deactivated')
                console.log("[Deactivate] Broadcast sent")  
            }
