...
async function getSessionCookie(sessionId: string) {
    const tokenRes = await request(app).get(`/api/csrf/token?session_id=${sessionId}`)
...
async function joinClassroom(classroomCode: string) {
    const joinRes = await request(app).get(
        `/api/classrooms/${classroomCode}/join?student_id=test-student`
    )
