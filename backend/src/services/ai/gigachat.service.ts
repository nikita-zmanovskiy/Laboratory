...
export class GigaChatService extends BaseAiService {
    ...
    async analyzeImage(prompt: string, imageBase64: string, systemPrompt?: string): Promise<any> {
        if (config.aiMock) return this.mockGenerate(prompt, imageBase64)
    
        console.log("[GigaChat] Uploading image for analysis...")
        const fileId = await this.uploadImage(imageBase64)
        console.log("[GigaChat] Image uploaded, file_id:", fileId)
    ...
    async generateWithImage(prompt: string, imageBase64: string, systemPrompt?: string): Promise<any> {
        if (config.aiMock) return this.mockGenerate(prompt, imageBase64)
    
        console.log("[GigaChat] Uploading image for img2img...")
        const fileId = await this.uploadImage(imageBase64)
        console.log("[GigaChat] Image uploaded, file_id:", fileId)
    ...
    private async uploadImage(base64Image: string): Promise<string> {
        ...
        console.log("[GigaChat Upload] Image size:", buffer.length, "bytes")
        ...
        try {
            ...
            console.log("[GigaChat Upload] Success, file_id:", response.data.id)
            return response.data.id
        } catch (error: any) {
            console.error("[GigaChat Upload] Error:", error.response?.status, JSON.stringify(error.response?.data))
            throw error
        }
    }
